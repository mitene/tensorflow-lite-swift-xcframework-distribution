...
public final class SignatureRunner {
    ...
    public var inputs: [String] {
        guard let inputs = _inputs else {
            let inputCount = Int(TfLiteSignatureRunnerGetInputCount(cSignatureRunner))
    ...
    public var outputs: [String] {
        guard let outputs = _outputs else {
            let outputCount = Int(TfLiteSignatureRunnerGetOutputCount(cSignatureRunner))
    ...
    public func invoke(with inputs: [String: Data]) throws {
        ...
        guard TfLiteSignatureRunnerInvoke(cSignatureRunner) == kTfLiteOk else {
    ...
    public func copy(_ data: Data, toInputNamed name: String) throws {
        guard let inputNameCString = name.cString(using: String.Encoding.utf8),
              let cTensor = TfLiteSignatureRunnerGetInputTensor(cSignatureRunner, inputNameCString)
        else {
    ...
    public func allocateTensors() throws {
        ...
        guard TfLiteSignatureRunnerAllocateTensors(cSignatureRunner) == kTfLiteOk else {
    ...
    private func tensor(named nameInSignature: String, withType type: TensorType) throws -> Tensor {
        ...
        case .input:
            UnsafePointer(
                TfLiteSignatureRunnerGetInputTensor(cSignatureRunner, nameInSignatureCString)
            )
        case .output:
            TfLiteSignatureRunnerGetOutputTensor(
                cSignatureRunner, nameInSignatureCString
            )
        }
